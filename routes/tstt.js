...
router.get("/content", async (req, res) => {
  ...
  if (!url) {
    return res.status(400).json({
      ...
      example: `${global.t}/api/content?url=https://dyrassa.com/امتحانات-إقليمية-في-الرياضيات/`
    });
  }
...
router.get("/content/media", async (req, res) => {
  ...
  if (!url) {
    return res.status(400).json({
      ...
      success: false,
      message: "⚠️ يرجى تقديم رابط الدرس بعد ?url=",
      example: `${global.t}/api/content/media?url=https://dyrassa.com/امتحانات-إقليمية-في-الرياضيات/`
    });
  }
...
router.get("/content/status", async (req, res) => {
  try {
    // اختبار اتصال بسيط
    const testUrl = "https://dyrassa.com/امتحانات-إقليمية-في-الرياضيات/";
...
module.exports = {
  path: "/api/tools",
  name: "dyrassa content extractor",
  type: "tools",
  url: `${global.t}/api/tools/content?url=https://dyrassa.com/امتحانات-إقليمية-في-الرياضيات/`,
  logo: "",
  description: "أداة استخراج محتوى الدروس من موقع dyrassa.com بجميع الأنواع (PDF، نص، صور، مختلط)",
  router
};
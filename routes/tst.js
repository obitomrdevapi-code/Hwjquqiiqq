const express = require("express");
const canvafy = require("canvafy");
const axios = require("axios");

const router = express.Router();

router.get("/levelup", async (req, res) => {
  const { profile, name, lvl1, lvl2, bg} = req.query;

  if (!profile ||!name ||!lvl1 ||!lvl2) {
    return res.status(400).json({
      status: 400,
      success: false,
      message: "❌ يجب توفير جميع البارامترات: profile, name, lvl1, lvl2",
});
}

  try {
    // تحميل الخلفية كـ Buffer
    const bgUrl = bg || "https://i.postimg.cc/FstkPXKk/1761952083345.jpg";
    const bgRes = await axios.get(bgUrl, { responseType: "arraybuffer"});
    const bgBuffer = Buffer.from(bgRes.data, "binary");

    const image = await new canvafy.LevelUp()
.setAvatar(profile)
.setBackground("buffer", bgBuffer) // ✅ استخدام buffer بدل رابط مباشر
.setUsername(name)
.setBorder("#000000")
.setAvatarBorder("#00ff00") // ✅ اللون الأخضر بدل الأحمر
.setOverlayOpacity(0.7)
.setLevels(Number(lvl1), Number(lvl2))
.build();

    res.setHeader("Content-Type", "image/png");
    res.send(image);
} catch (err) {
    console.error("[ERROR] فشل إنشاء الصورة:", err.message);
    res.status(500).json({
      status: 500,
      success: false,
      message: "حدث خطأ أثناء توليد الصورة 🚫",
      error: err.message,
});
}
});

module.exports = {
  path: "/api/tools",
  name: "Level Up Image",
  type: "tools",
  url: `${global.t}/api/tools/levelup?profile=رابط&name=اسم&lvl1=1&lvl2=2&bg=رابط_الخلفية`,
  logo: "https://i.ibb.co/m53WF9N/avatar-contact.png",
  description: "تولة",
  router,
};
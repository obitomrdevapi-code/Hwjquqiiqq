...
const base = "https://api.allorigins.win/raw?url=https://temporary-phone-number.com";

async function fetchCountries() {
  const res = await fetch(`${base}/countrys/`);
  ...
  $("a.checkout-box").each((_, el) => {
    ...
    result.push({ name, link: base + href});
});
...
async function fetchNumbers(url) {
  ...
  $(".col-sm-6.col-md-4.col-lg-3.col-xs-12").each((_, el) => {
    ...
    result.push({ number, link: base + href});
});
...
module.exports = {
  ...
  type: "tools",
  url: "https://obito-mr-apis.vercel.app/api/tools/tst?type=country",
  logo: "https://qu.ax/numlogo.png",
  description: "Get countries, numbers, and messages from temporary-phone-number.com",
  router,
};
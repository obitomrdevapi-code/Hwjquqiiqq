...
async function fbDownloader(url) {
  ...
  $('a.download-link-fb').each((_, el) => {
    ...
    if (link) links.push({ جودة: quality, صيغة: format, رابط: link });
  });
...
router.get("/facebook", async (req, res) => {
  ...
  try {
    ...
    if (!result.حالة || !result.روابط || result.روابط.length === 0) {
    ...
    res.json({
      ...
      data: {
        الرابط_الأصلي: url,
        الرابط_المعدل: url.includes('/share/r/') ? url.replace('/share/r/', '/share/v/') : url,
        العنوان: result.عنوان,
        المدة: result.مدة,
        الصورة_المصغرة: result.صورة_مصغرة,
        عدد_الروابط: result.روابط.length,
        الروابط: result.روابط
      }
    });
...
module.exports = {
  ...
  type: "downloader",
  url: `${global.t}/api/download/facebook?url=https://www.facebook.com/share/r/17VdF8kuf5/`,
  logo: "",
  description: "تحميل فيديوهات من الفيسبوك",
  router
};
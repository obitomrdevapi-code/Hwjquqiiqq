...
async function fetchCountries() {
  const res = await fetch("https://temporary-phone-number.com/countrys/");
...
async function fetchMessages(url) {
  ...
  $(".direct-chat-msg.left").each((_, el) => {
    ...
    const codeMatch = text.match(/(?:code|كود|رمز|OTP)[^\d]*(\d{4,8})/i) || text.match(/\b\d{4,8}\b/);
    const code = codeMatch? codeMatch[1] || codeMatch[0]: null;
